...
public abstract class KeycloakTestContainers {
    ...
    static {
        keycloak = new KeycloakContainer()
                .withRealmImportFile("client.realm-export.json");
        keycloak.start();
    }
    ...
    static void registerProperties(DynamicPropertyRegistry registry) {
        registry.add("spring.security.oauth2.resourceserver.jwt.issuer-uri",
                () -> keycloak.getAuthServerUrl() + "/realms/client-app");
        registry.add("keycloak.auth-server-url", keycloak::getAuthServerUrl);
        registry.add("keycloak.realm", () -> "client-app");
        ...
        registry.add("keycloak.login-server-url",
                () -> keycloak.getAuthServerUrl() + "/realms/client-app/protocol/openid-connect/token");
